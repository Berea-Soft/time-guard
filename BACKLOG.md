...
- [ ] `fix(docs-app): sandboxes StackBlitz instalan @bereasoftware/time-guard@latest de npm, no el código local`
  Objetivo: Descubierto al verificar `LocalesDemo`: su sandbox falla por completo (`SyntaxError: ... does not provide an export named 'loadAllLocales'`, preview en blanco) porque `TIME_GUARD_VERSION = 'latest'` instala la ÚLTIMA VERSIÓN PUBLICADA en npm, que todavía no incluye `loadAllLocales`/`getAvailableLocales` ni los fixes de `diff()`/`humanize()`/etc. de esta sesión. `PluginsDemo` no crashea pero muestra valores viejos ("0 years ago") por el mismo motivo — cualquier demo que use una API agregada/corregida después del último `npm publish` mostrará comportamiento obsoleto o roto en su sandbox, sin relación con el código local (que está correcto). Esto afecta TODOS los sandboxes (incluidos React/Angular/Svelte/Solid/Qwik), no solo los 7 nuevos — es estructural al enfoque "instalar el paquete real desde npm".
  Criterio de aceptación: n/a todavía — requiere decisión del usuario (ver próxima conversación): publicar una nueva versión a npm es lo que resuelve esto de raíz; no hay fix de código que lo evite mientras el sandbox dependa del registro público.
  **Pendiente:** Sin acción tomada — reportado al usuario, no es un bug de código sino de versión publicada vs. código local.
